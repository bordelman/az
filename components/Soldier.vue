<template>
	<section class="soldier">
		<div>
			...
			<input v-else type="email" v-model="soldier.personalNumber" />
		</div>
		<div>
			Hodnost: <span v-if="!edit">{{ soldier.rank.rank }}</span>
			<select v-else v-model="soldier.rank.id">
				<option v-for="rank of ranks" :key="'rank-' + rank.id" :value="rank.id">
					{{ rank.rank }}
				</option>
			</select>
		</div>
		<div>
			...
			<input v-else type="text" v-model="soldier.firstname" />
		</div>
		<div>
			Příjmení: <span v-if="!edit">{{ soldier.lastname }}</span>
			<input v-else type="text" v-model="soldier.lastname" />
		</div>
		<div>
		...
		<div>
			Email:
			<a v-if="!edit" :href="'mailto:' + soldier.email">{{ soldier.email }}</a>
			<input v-else type="email" v-model="soldier.email" />
		</div>
		<div>
			Mobil:
			...
			<input v-else type="tel" v-model="soldier.mobile" />
		</div>
		<div>
			Pozice: <span v-if="!edit">{{ soldier.position.position }}</span>
			<select v-else v-model="soldier.position.id">
				<option v-for="position of positions" :key="'positions-' + position.id" :value="position.id">
					{{ position.position }}
				</option>
			</select>
		</div>
		<div>
			...
			<select v-else v-model="soldier.company">
				<option value="1">1</option>
			</select>
		</div>
		<div v-if="soldier.platoon || edit">
			Četa: <span v-if="!edit">{{ soldier.platoon }}</span>
			<select v-else v-model="soldier.platoon">
				<option value=""></option>
				<option value="1">1</option>
		...
		<div v-if="soldier.squad || edit">
			...
			<select v-else v-model="soldier.squad">
				<option value=""></option>
				<option value="1">1</option>
				<option value="2">2</option>
				<option value="3">3</option>
		...
		<section class="drills">
			...
			<section class="lists">
				<section class="attend">
					<h2>
						Zúčastním se <span class="count">({{ nominationsPresent.length }})</span>
					</h2>
					<ul class="nominations">
						<li class="nomination" v-for="(nomination, index) of nominationsPresent" :key="'nomination-' + index">
							<NuxtLink :class="['nomination-link', EAttendance[nomination.status]]"
				...
				<section class="skipped">
					<h2>
						Nezúčastním se <span class="count">({{ nominationsAbsent.length }})</span>
					</h2>
					<ul class="nominations">
						<li class="nomination" v-for="(nomination, index) of nominationsAbsent" :key="'nomination-' + index">
							<NuxtLink :class="['nomination-link', EAttendance[nomination.status]]"
								:to="'/drills/' + nomination.drill.id">
								{{ nomination.drill.name }} ({{
									new Date(nomination.drill.dateFrom).toLocaleDateString()
				...
				<section class="not-responded">
					<h2>
						Nevyjádřil jsem se <span class="count">({{ nominationsNotResponded.length }})</span>
					</h2>
					<ul class="nominations">
						<li class="nomination" v-for="(nomination, index) of nominationsNotResponded" :key="'nomination-' + index">
							<NuxtLink :class="['nomination-link', EAttendance[nomination.status]]"
								:to="'/drills/' + nomination.drill.id">
								{{ nomination.drill.name }} ({{
									new Date(nomination.drill.dateFrom).toLocaleDateString()
...
const
	...
	nominationsNotResponded = nominations.filter(nomination => nomination.status === EAttendance.NotResponded),
	nominationsPresent = nominations.filter(nomination => nomination.status === EAttendance.Present),
	nominationsAbsent = nominations.filter(nomination => nomination.status === EAttendance.Absent);
...
async function callEditSoldier() {
	...
	try {
		Object.assign(soldier.value, await editSoldier({
